...
public class Scanner {
	...
	*/
	...
	int[] initLineStarts() {
		...
		int[] ret = new int[lineStarts.size()];
	    Iterator<Integer> iterator = lineStarts.iterator();
	...
	*/
	...
	private enum State {START, AFTER_DIV, AFTER_COLON, AFTER_LT, AFTER_GT, AFTER_EX, AFTER_MULTI, AFTER_EQUALS, AFTER_IDENTIFIER, AFTER_DIGIT,
		                AFTER_COMMENT, AFTER_FLOAT};  //TODO:  this is incomplete

	 
	//TODO: Modify this to deal with the entire lexical specification
	public Scanner scan() throws LexicalException {
		
		HashMap<String, Kind> Keywords_List = new HashMap<String, Kind>();
		Keywords_List.put("Z", Kind.KW_Z);
		Keywords_List.put("default_width", Kind.KW_default_width);
		Keywords_List.put("default_height", Kind.KW_default_height);
		Keywords_List.put("width", Kind.KW_width);
		Keywords_List.put("height", Kind.KW_height);
		Keywords_List.put("abs", Kind.KW_abs);
		Keywords_List.put("sin", Kind.KW_sin);
		Keywords_List.put("cos", Kind.KW_cos);
		Keywords_List.put("atan", Kind.KW_atan);
		Keywords_List.put("log", Kind.KW_log);
		Keywords_List.put("image", Kind.KW_image);
		Keywords_List.put("int", Kind.KW_int);
		Keywords_List.put("float", Kind.KW_float);
		Keywords_List.put("boolean", Kind.KW_boolean);
		Keywords_List.put("filename", Kind.KW_filename);
		Keywords_List.put("red", Kind.KW_red);
		Keywords_List.put("blue", Kind.KW_blue);
		Keywords_List.put("green", Kind.KW_green);
		Keywords_List.put("alpha", Kind.KW_alpha);
		Keywords_List.put("while", Kind.KW_while);
		Keywords_List.put("if", Kind.KW_if);
		Keywords_List.put("sleep", Kind.KW_sleep);
		Keywords_List.put("true", Kind.BOOLEAN_LITERAL);
		Keywords_List.put("false", Kind.BOOLEAN_LITERAL);
		Keywords_List.put("show", Kind.KW_show);
		Keywords_List.put("write", Kind.KW_write);
		Keywords_List.put("to", Kind.KW_to);
		Keywords_List.put("input", Kind.KW_input);
		Keywords_List.put("from", Kind.KW_from);
		Keywords_List.put("cart_x", Kind.KW_cart_x);
		Keywords_List.put("cart_y", Kind.KW_cart_y);
		Keywords_List.put("polar_a", Kind.KW_polar_a);
		Keywords_List.put("polar_r", Kind.KW_polar_r);
				...
				case START: {
					...
					switch (ch) {
                        ...
                        case '>':
                            ...
                            break;
                        case '!':
                            state = State.AFTER_EX;
                            pos++;
                            break;
                        case '*':
                            state = State.AFTER_MULTI;
                            pos++;
                            break;
                        case '=':
                    ...
                    if(Character.isDigit(ch)) {
                        pos++;
                    	   ...
                    	   state = State.AFTER_FLOAT;
                    	   	...
                    	   	BigInteger intValue = new BigInteger(String.copyValueOf(chars, startPos, pos-startPos));
            					if(BigInteger.valueOf(Integer.MAX_VALUE).compareTo(intValue) < 0){
            						error(pos, line(pos), posInLine(pos), "integer limit Illegal");
            					}
            					else{
                ...
                case AFTER_FLOAT: {
                    if(Character.isDigit(ch)) {
                        pos++;
                    	   ...
                    	   Token t = new Token(Kind.FLOAT_LITERAL, startPos, pos - startPos);
                    	   Float floatValue  = t.floatVal();
                    	   if(Float.isFinite(floatValue)) {
                    		   tokens.add(t);
                    		   state = State.START;
                    	   } else {
                    	   	error(pos, line(pos), posInLine(pos), "Illegal");
                    	   }
                    }
                }
                break;
                case AFTER_IDENTIFIER: {
                    if(Character.isAlphabetic(ch) || Character.isDigit(ch) || ch == '$' || ch == '_') {
                        pos++;
                    	   ...
                    	   String checkKey = String.copyValueOf(chars, startPos, pos - startPos);
                    	   if(Keywords_List.containsKey(checkKey)) {
                    	       tokens.add(new Token(Keywords_List.get(checkKey), startPos, pos - startPos));
                    	   } else {
